...
// 2. FLOR — pétalos radiales que se abren con la voz
function flowerPoint(i, total, time = 0, amplitude = 0) {
    const t = i / total;
    const angle = t * Math.PI * 2 * 5; // 5 pétalos
    const radius = 0.7 * (1 + amplitude * 0.5); // se expande con la voz
    const petalShape = Math.sin(angle) * 0.3 + 0.7;
    const x = Math.cos(angle) * radius * petalShape;
    const z = Math.sin(angle) * radius * petalShape;
    const y = Math.sin(angle * 2) * 0.2 + amplitude * 0.15;
    // Añadir vibración
    const vib = amplitude * 0.05 * Math.sin(angle * 10 + time * 20);
    return new THREE.Vector3(x + vib, y + vib * 0.5, z + vib);
}

// 3. CONCHA — espiral cónica con profundidad
function shellPoint(i, total) {
    const t = i / total;
    const turns = 3.5;
    const angle = t * Math.PI * 2 * turns;
    const radius = (1 - t) * 0.85;
    const x = Math.cos(angle) * radius;
    const z = Math.sin(angle) * radius * 0.7;
    const y = t * 0.45 - 0.15; // inclinación
    const conchDepth = Math.sin(angle * 2) * 0.05 * (1 - t);
    return new THREE.Vector3(x, y + conchDepth, z);
}
...
for (let i = 0; i < PARTICLE_COUNT; i++) {
    ...
    // Decidir color basado en ángulo y altura
    let colorRef;
    ...
    // Variación sutil entre partículas
...
// =============================================
// DODECAEDRO PROTECTOR (sin cambios)
// =============================================
...
// GlowShell orbital (sin cambios, pero se mantiene)
