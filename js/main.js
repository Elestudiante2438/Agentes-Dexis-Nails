...
// =============================================
// 3.5 MALLA NEURAL JARVIS — capa exterior azul
// =============================================
const NEURAL_NODE_COUNT = 130;
const NEURAL_RADIUS     = 5.85;   // justo afuera de la bubble (r=5.5)
const NEURAL_CONNECT_DIST = 2.8;  // distancia máx para trazar arista
...
for (let i = 0; i < NEURAL_NODE_COUNT; i++) {
    // distribución uniforme en esfera (método Fibonacci)
    const phi   = Math.acos(1 - 2 * (i + 0.5) / NEURAL_NODE_COUNT);
    const theta = Math.PI * (1 + Math.sqrt(5)) * i;
    // añadir jitter para que no sea perfectamente uniforme
...
const edgePoints = [];
const edgePairs  = []; // para animar por segmento
for (let i = 0; i < NEURAL_NODE_COUNT; i++) {
...
// Geometría de líneas con colores por vértice para pulsos
const edgeGeo = new THREE.BufferGeometry().setFromPoints(edgePoints);
const edgeSegCount = edgePoints.length / 2;

// Array de opacidades para animar — una por segmento (2 vértices)
const alphaArr = new Float32Array(edgePoints.length);
alphaArr.fill(0.25);
edgeGeo.setAttribute('alpha', new THREE.BufferAttribute(alphaArr, 1));

// Material de líneas con vertex colors implícito vía uniforme global
...
const neuralLines = new THREE.LineSegments(edgeGeo, neuralLineMat);
neuralGroup.add(neuralLines);

// --- Pulsos eléctricos: algunos segmentos brillan en secuencia ---
const pulseStates = edgePairs.map((_, i) => ({
    phase: Math.random() * Math.PI * 2,
    speed: 0.8 + Math.random() * 2.5,
    amp:   0.5 + Math.random() * 0.5,
}));

// Variables de intensidad para reaccionar al estado de voz
let neuralIntensity = 1.0;   // 1=silencio, 1.8=escuchando, 2.5=hablando
...
function animate() {
    ...
    // --- MALLA NEURAL: pulsos eléctricos ---
    // La malla gira muy lentamente sobre sí misma
    neuralGroup.rotation.y += 0.0005;
    neuralGroup.rotation.x += 0.00015;

    // Pulso global de opacidad base
    const neuralBase = (0.18 + Math.sin(time * 1.2) * 0.06) * neuralIntensity;
    neuralLineMat.opacity = Math.min(neuralBase, 0.9);

    // Pulso individual por segmento: bright flashes
    // Actualizamos en lotes para performance (cada 2 frames)
    if (Math.round(time * 60) % 2 === 0) {
        for (let s = 0; s < edgePairs.length; s++) {
            ...
            // Los primeros 20% de segmentos tienen pulso más brillante (electricidad random)
            if (s % 7 === 0 && glow > 0.6) {
                // flash eléctrico intenso — cambia color momentáneamente
                // (se refleja en el material global; para diferenciación real necesitaríamos ShaderMaterial)
                neuralLineMat.opacity = Math.min(neuralBase + glow * 0.5 * neuralIntensity, 1.0);
            }
        }
    }

    // Tamaño de nodos pulsa con el beat del núcleo
    const nodePulse = 1 + beat * 0.12 * neuralIntensity;
    nodeMat.emissiveIntensity = 1.4 * neuralIntensity + Math.sin(time * 3.1) * 0.4;
    neuralGroup.children.forEach((child, i) => {
    ...
    if (!listeningActive && !isSpeaking && !permanentColor) {
        ...
        // malla neural cicla el mismo hue que las partículas
