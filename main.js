...
function setTopView() {
    camera.position.set(window.innerWidth < 768 ? 3 : 4, window.innerWidth < 768 ? 5 : 6, window.innerWidth < 768 ? 7 : 8);
    controls.target.set(0, 0.5, 0);
    controls.update();
...
const bubbleMat = new THREE.MeshStandardMaterial({ color: 0x000000, emissive: 0x000000, metalness: 0.85, roughness: 0.1, transparent: true, opacity: 0.08, side: THREE.DoubleSide, depthWrite: false });
const bubble = new THREE.Mesh(bubbleGeo, bubbleMat);
bubble.position.set(0, 0.5, 0);
scene.add(bubble);

const coreMat = new THREE.MeshStandardMaterial({ color: 0x44cc88, emissive: 0x22aa55, emissiveIntensity: 1.2, metalness: 1.0, roughness: 0.0 });
const core = new THREE.Mesh(new THREE.SphereGeometry(0.55, 64, 64), coreMat);
core.position.set(0, 0.5, 0);
core.castShadow = true;
scene.add(core);

const glowMat = new THREE.MeshStandardMaterial({ color: 0x88ffcc, emissive: 0x44ddaa, emissiveIntensity: 0.4, metalness: 0.6, roughness: 0.3, transparent: true, opacity: 0.3, depthWrite: false });
...
const dodecaMat = new THREE.MeshStandardMaterial({ color: 0x88aaff, emissive: 0x112244, emissiveIntensity: 0.6, metalness: 0.9, roughness: 0.2, transparent: true, opacity: 0.55 });
...
const vertexMat = new THREE.MeshStandardMaterial({ color: 0xffaa88, emissive: 0x884422, emissiveIntensity: 0.8, metalness: 0.9 });
...
function makeRing(radius, tube, color, emissive, rotX, rotZ) {
    const mat = new THREE.MeshStandardMaterial({ color, emissive, emissiveIntensity: 0.8, metalness: 0.95, roughness: 0.05, transparent: true, opacity: 0.9 });
...
const particleMat = new THREE.PointsMaterial({ color: 0x88aaff, size: 0.022, transparent: true, blending: THREE.AdditiveBlending, depthWrite: false });
...
const starsBg = new THREE.Points(starGeo, new THREE.PointsMaterial({ color: 0xffffff, size: 0.08, transparent: true, opacity: 0.7, blending: THREE.AdditiveBlending, depthWrite: false }));
scene.add(starsBg);
...
function makePlanetTexture(size, opts = {}) {
    const { baseColor = [0.2, 0.5, 0.8], darkColor = [0.05, 0.1, 0.3], noiseScale = 4, poles = false, poleColor = [0.9, 0.95, 1.0], cloudy = false, cloudColor = [1, 1, 1], lava = false, hasContinents = true } = opts;
...
planetas.forEach((def, idx) => {
    ...
    const planet = new THREE.Mesh(new THREE.SphereGeometry(def.size, 64, 64), new THREE.MeshStandardMaterial({ map: tex, metalness: 0.1, roughness: 0.8, emissive: new THREE.Color(def.color), emissiveIntensity: 0.04 }));
    ...
    const atmo = new THREE.Mesh(new THREE.SphereGeometry(def.size * 1.12, 32, 32), new THREE.MeshStandardMaterial({ color: def.atmoColor, emissive: def.atmoColor, emissiveIntensity: 0.2, transparent: true, opacity: 0.15, side: THREE.BackSide, depthWrite: false, blending: THREE.AdditiveBlending }));
    planet.add(atmo);
    if (def.rings) {
        const rMesh = new THREE.Mesh(new THREE.TorusGeometry(def.size * 1.7, def.size * 0.4, 2, 120), new THREE.MeshStandardMaterial({ color: 0xffcc77, emissive: 0x553300, emissiveIntensity: 0.3, metalness: 0.4, roughness: 0.7, transparent: true, opacity: 0.6, side: THREE.DoubleSide }));
        rMesh.rotation.x = Math.PI / 2;
        planet.add(rMesh);
    }
    if (def.moon) {
        const moonTex = makePlanetTexture(128, { baseColor: [0.55, 0.55, 0.55], darkColor: [0.2, 0.2, 0.22], noiseScale: 5, hasContinents: true });
        const moon = new THREE.Mesh(new THREE.SphereGeometry(def.moon.size, 16, 16), new THREE.MeshStandardMaterial({ map: moonTex, roughness: 0.95, metalness: 0.05 }));
...
function createShip(color = 0x88aaff) {
    ...
    const body = new THREE.Mesh(new THREE.ConeGeometry(0.06, 0.13, 8), new THREE.MeshStandardMaterial({ color, emissive: color, emissiveIntensity: 0.12, metalness: 0.85, roughness: 0.15 }));
    body.rotation.x = Math.PI / 2;
    group.add(body);
    const lWing = new THREE.Mesh(_wingGeo, _wingMat);
    lWing.position.set(-0.065, 0, 0.01);
    const rWing = new THREE.Mesh(_wingGeo, _wingMat);
    rWing.position.set(0.065, 0, 0.01);
...
for (let i = 0; i < 12; i++) {
    ...
    ship.userData = { orbitRadius: 1.6 + Math.random() * 0.8, orbitSpeed: -(0.3 + Math.random() * 0.4), angle: (i / 12) * Math.PI * 2, yAmp: 0.1 + Math.random() * 0.15, yFreq: 0.5 + Math.random() * 1.0 };
    scene.add(ship);
    ships.push(ship);
}
...
const agentColorsMap = { Tejedora: 0xFFD700, Kai: 0x3399FF, 'Quántor': 0x33CC66, Memoria: 0xFF3333, Valorador: 0xFF8800, Faro: 0xDDEEFF };
document.getElementById('statusMsg')?.style.setProperty('display', 'none');
const _edgeColor = new THREE.Color();

function setNucleusColor(hex) { _edgeColor.setHex(hex); coreMat.color.copy(_edgeColor); coreMat.emissive.copy(_edgeColor).multiplyScalar(0.6); dodecaMat.color.copy(_edgeColor); particleMat.color.copy(_edgeColor); vertexMat.color.copy(_edgeColor); }
function setListeningMode() { bubbleMat.emissive.setHex(0x3399FF); bubbleMat.emissiveIntensity = 0.8; bubbleMat.color.setHex(0x3399FF); ring1.mat.emissiveIntensity = 1.2; ring2.mat.emissiveIntensity = 1.2; ring3.mat.emissiveIntensity = 1.0; currentRingSpeed = normalRingSpeed; }
function setSpeakingMode() { bubbleMat.emissive.setHex(0xFFAA44); bubbleMat.emissiveIntensity = 1.0; bubbleMat.color.setHex(0xFFAA44); ring1.mat.emissiveIntensity = 1.6; ring2.mat.emissiveIntensity = 1.6; ring3.mat.emissiveIntensity = 1.4; currentRingSpeed = normalRingSpeed * 1.8; setTimeout(() => { if (listeningActive) setListeningMode(); else setSilenceMode(); }, 1200); }
function setSilenceMode() { bubbleMat.emissive.setHex(0x000000); bubbleMat.emissiveIntensity = 0.0; bubbleMat.color.setHex(0x000000); ring1.mat.emissiveIntensity = 0.7; ring2.mat.emissiveIntensity = 0.7; ring3.mat.emissiveIntensity = 0.7; currentRingSpeed = normalRingSpeed; }

async function getDexiResponse(userText) {
    const lower = userText.toLowerCase();
    if (lower.includes('dexis')) return { respuesta: "Soy Dexis, tu asistente. ¿En qué te ayudo?", agenteNombre: 'Dexis' };
    if (lower.includes('ayuda')) return { respuesta: "Claro. Puedes reservar servicios de manicura, pedicura, podología, uñas de gel, faciales, o consultar por colonias árabes. ¿Qué necesitas?", agenteNombre: 'Dexis' };
    ...
    if (lastAgentColor !== newColor) { lastAgentColor = newColor; permanentColor = newColor; setNucleusColor(newColor); }
...
function speakResponse(text) { if (!text || isSpeaking) return; isSpeaking = true; const u = new SpeechSynthesisUtterance(text); u.lang = 'es-CO'; u.rate = 0.9; u.onend = u.onerror = () => { isSpeaking = false; }; window.speechSynthesis.cancel(); window.speechSynthesis.speak(u); }
function startListening() { if (listeningActive) return; const SR = window.SpeechRecognition || window.webkitSpeechRecognition; if (!SR) return; recognition = new SR(); recognition.continuous = true; recognition.lang = 'es-CO'; recognition.onstart = () => { listeningActive = true; setListeningMode(); }; recognition.onend = () => { if (listeningActive) stopListening(); }; recognition.onerror = () => stopListening(); recognition.onresult = (event) => { let finalText = ''; for (let i = event.resultIndex; i < event.results.length; i++) if (event.results[i].isFinal) finalText += event.results[i][0].transcript + ' '; if (finalText) { setSpeakingMode(); processUserText(finalText.trim()); setTimeout(() => { if (listeningActive) setListeningMode(); else setSilenceMode(); }, 1000); } }; recognition.start(); }
function stopListening() { recognition?.stop(); recognition = null; listeningActive = false; setSilenceMode(); }

window.addEventListener('keydown', (e) => { const k = e.key.toLowerCase(); if (k === 'e') { e.preventDefault(); startListening(); } else if (k === 'm') { e.preventDefault(); stopListening(); } else if (k === 'r') { e.preventDefault(); setTopView(); } });
...
function animate() {
    ...
    dodecahedron.position.set(core.position.x + Math.sin(time * 0.7) * 0.015, core.position.y + Math.sin(time * 0.5) * 0.01, core.position.z + Math.sin(time * 0.9) * 0.02);
    ...
    ships.forEach(ship => { const d = ship.userData; d.angle += d.orbitSpeed * 0.008; ship.position.set(dodecahedron.position.x + Math.cos(d.angle) * d.orbitRadius, dodecahedron.position.y + Math.sin(d.angle * d.yFreq) * d.yAmp, dodecahedron.position.z + Math.sin(d.angle) * d.orbitRadius); ship.rotation.y = d.angle; ship.rotation.x = Math.sin(d.angle * 2) * 0.18; });
    planetFigures.forEach(planet => { const d = planet.userData; d.angle += d.speed; planet.position.set(Math.cos(d.angle) * d.distance, 0.5 + Math.sin(d.angle * 2) * 0.06, Math.sin(d.angle) * d.distance); planet.rotation.y += 0.004; if (d.moon) { const md = d.moon.userData; md.angle += md.speed; d.moon.position.set(Math.cos(md.angle) * md.dist, Math.sin(md.angle * 0.5) * 0.15, Math.sin(md.angle) * md.dist); } });
    if (!listeningActive && !isSpeaking && !permanentColor) { particleHue = (particleHue + 0.004) % 1; particleMat.color.setHSL(particleHue, 1, 0.6); }
    if (permanentColor) { const t = 0.3 + Math.sin(time * 6) * 0.4; _edgeColor.setHex(permanentColor); edgesMat.color.setRGB(_edgeColor.r * t, _edgeColor.g * t, _edgeColor.b * t); }
...
function initChat() {
    ...
    if (!chatInput) { console.warn('Chat no encontrado en el DOM'); return; }
    chatSendBtn.addEventListener('click', sendChatMessage);
    chatInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') { e.preventDefault(); sendChatMessage(); } });
    chatInput.addEventListener('focus', () => { setTimeout(() => { window.scrollTo(0, 0); document.body.scrollTop = 0; }, 100); });
...
async function sendChatMessage() {
    ...
    try {
        if (!window.Dexis || typeof window.Dexis.responder !== 'function') throw new Error('Dexis no está disponible');
        const respuesta = await window.Dexis.responder(message);
        addChatMessage(respuesta, 'dexis');
    } catch (error) { console.error('Error al llamar a Dexis:', error); addChatMessage('Lo siento, hubo un error. Intenta de nuevo.', 'dexis'); }
